...
const char* mqtt_client_id_base     = "TVLiftMega2560";
const char* mqtt_topic_status       = "domotica/tv_lift/status";
const char* mqtt_topic_rms          = "domotica/tv_lift/rms";
...
const unsigned long arduinoOfflineTimeoutMs = 30000;
...
//  Pubblicazione MQTT
// ──────────────────────────────────────────────────────────
String mapLiftStatusForTopic() {
  if (remoteState.stateCode != "IDLE") return "in movimento";
  if (remoteState.tvPosition == "OPEN") return "aperta";
  if (remoteState.tvPosition == "CLOSED") return "chiusa";
  return "in movimento";
}
...
void publishStatus(bool force) {
  ...
  String statusValue = mapLiftStatusForTopic();
  String rmsValue = String(remoteState.currentRms, 3);

  bool statusOk = client.publish(mqtt_topic_status, statusValue.c_str(), true);
  bool rmsOk = client.publish(mqtt_topic_rms, rmsValue.c_str(), true);
  if (!statusOk || !rmsOk) {
    Serial.println(String("MQTT publish failed: status=") + (statusOk ? "OK" : "ERR") + ", rms=" + (rmsOk ? "OK" : "ERR"));
  }
  lastPublish = now;
}
...
void handleRoot() {
  ...
  html += "<section class='card'><div class='k'>Manutenzione ESP8266</div><div class='mini' style='margin:8px 0'>Aggiorna il firmware del gateway via rete caricando un file .bin. Percorso diretto: /update</div>";
  html += "<a class='btn btn-main' href='/update'>Aggiorna firmware ESP (OTA)</a></section>";

  html += "<section class='card'><div class='k'>Manutenzione Mega2560</div><div class='mini' style='margin:8px 0'>Upload firmware .hex e validazione web. Percorso diretto: /mega-update</div>";
  ...
  html += "<article class='card'><h3 style='margin:0 0 8px'>Tempi attuatori e ritardi TV (ms)</h3><div class='mini' style='margin-bottom:8px'>Qui trovi anche i 5 secondi prima dell'apertura e i 20 secondi prima della chiusura</div><form action='/settimes' method='POST' data-live-lock='1'>";
  ...
  html += "<div class='row'><label>Ritardo prima apertura TV ON (ms sopra soglia)</label><input id='ton' type='number' min='500' max='120000' name='ton' value='" + String(remoteState.tvOnConfirmMs) + "'></div>";
  html += "<div class='row'><label>Ritardo prima chiusura TV OFF (ms sotto soglia)</label><input id='toff' type='number' min='1000' max='300000' name='toff' value='" + String(remoteState.tvOffConfirmMs) + "'></div>";
...
void setup() {
  ...
  mqttClientId = String(mqtt_client_id_base) + "-" + String(ESP.getChipId(), HEX);
